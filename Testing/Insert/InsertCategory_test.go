...
func TestInsertCategory(t *testing.T) {
	...
	resp, err := http.Post("http://localhost:8080/category/add", "application/json", bytes.NewBuffer(data))
	if err != nil {
	...
	if string(body) != "\"Insert to Category done\"\n" {
		t.Errorf("Expected response body 'Insert to category done', got '%s'", string(body))
	}
